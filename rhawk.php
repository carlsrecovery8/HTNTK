<?php 
 ...
 function update() 
     { 
         echo "\n\e[91m\e[1m[+] HTNTK UPDATE UTILITY [+]\nUpdate in progress, please wait...\n\n$cln"; 
         system("git fetch origin && git reset --hard origin/master && git clean -f -d"); 
         echo $bold . $fgreen . "[i] Job finished successfully! Please Restart HTNTK \n" . $cln; 
         exit; 
     } 
 ...
 elseif ($ip == "fix") 
   { 
     ...
     echo $bold . $fgreen . "[i] Job finished successfully! Please Restart RED HTNTK \n"; 
     exit; 
   } 
 elseif ($ip == "update") 
   { 
       ...
       { 
         ...
         elseif ($scan == "2") 
           { 
             ...
             foreach ($geoips as $geoip) 
               { 
                 echo $bold . $lblue . "[CHK-IP]$green $geoip \n"; 
               } 
             echo "\n\n"; 
             echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n"; 
             trim(fgets(STDIN, 1024)); 
         ...
         elseif ($scan == "8") 
           { 
             ...
             if (count($array) > 0) 
               { 
                 userinput("Do You Want HTNTK To Detect CMS Of The Sites? [Y/N]"); 
             ...
             if (strpos($srccd, "wp-content") !== false) 
               { 
                 ...
                 if ($wp_version != "") 
                   ...
                   { 
                     $reallink = $ipsl . $ip; 
                     echo $red . "Failed \n\n[!] HTNTK could not determine the WordPress version of the target!"; 
                     echo "\n\n"; 
                     echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n"; 
                     trim(fgets(STDIN, 1024)); 
                     goto scanlist; 
         ...
         elseif ($scan == 'U' || $scan == 'u') 
           { 
             echo "\n\n" . $bold . $yellow . "-[ HTNTK Update Corner]-\n\n" . $cln; 
             ...
             elseif ($rhversion == $latestversion) 
               { 
                 echo $bold . $fgreen . "\n[i] You are already running the latest version of HTNTK. \n\n" . $cln; 
                 goto scanlist; 
               } 
             else 
               { 
         ...
         elseif ($scan == "A" || $scan == "a") 
           { 
             ...
             echo "\n\n$bold" . $lblue . "C H K  I P  L O O K  U P\n"; 
             echo "========================="; 
             echo "\n\n$cln"; 
             $urlgip    = "http://api.hackertarget.com/geoip/?q=" . $lwwww; 
             $resultgip = readcontents($urlgip); 
